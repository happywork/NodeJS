define([
    ...
    function Engine() {
        ...
        self.ws.on('homepage_event', function(nTotalMemberProfits) {
            // WRT
            self.nTotalMemberProfits = nTotalMemberProfits;
            self.trigger('homepage_event', nTotalMemberProfits);
        });
        /**
        ...
        self.ws.on('warning_point_updated', function(data) {
            self.trigger('warning_point_updated', data);
        });
    }
    ...
    Engine.prototype.finishRound = function(currentTime, currentPoint) {
        //if(this.gameState === 'STARTING' || )
        if(currentPoint == undefined || currentPoint <= 1)
            return;
